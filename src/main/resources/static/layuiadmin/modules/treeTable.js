...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.initOptions = function (opt) {
        ...
        } else if ('add' === this.options.toolbar) {
            this.options.toolbar =
                '<div class="layui-btn-container">' +
                '    <button class="layui-btn layui-btn-sm" lay-event="add" id="add" style="display: none; float: left">新增标签</button>' +
                '</div>'
                // '<div><div class="ew-tree-table-tool-item" title="添加" lay-event="add">' +
                // '   <i class="layui-icon layui-icon-add-1"></i>' +
                // '</div></div>'
        } else if ('add,bmlz' === this.options.toolbar) {
            this.options.toolbar =
                '<div class="layui-btn-container">' +
                '    <button class="layui-btn layui-btn-sm" lay-event="add" id="add" style="display: none; float: left">新增标签</button>' +
                '    <div id="bmlzDiv" style="display: none; float: left">' +
                '       <label class="layui-form-label" style="line-height: 30px; padding: 0 0 0 20px;">部门流转</label>\n' +
                '       <div class="layui-input-block" style="height: 30px; min-height: 30px">\n' +
                '         <input type="checkbox" id="bmlz" name="bmlz" lay-skin="switch" lay-filter="switch" lay-text="开|关">\n' +
                '       </div>' +
                '    </div>' +
                '</div>'
            // '<div><div class="ew-tree-table-tool-item" title="添加" lay-event="add">' +
